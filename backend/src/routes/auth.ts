...
router.post('/oauth/register', asyncHandler(async (req: Request, res: Response) => {
  ...
  const userId = id; // Use the OAuth provider's user ID
  const businessId = uuidv4();
  ...
  const { error: userError } = await supabaseAdmin
    ...
    .insert({
      id: userId,
      email,
      password_hash: `oauth_${provider}_${Date.now()}`, // Placeholder for OAuth users
    });
  ...
  if (businessError) {
    ...
    logger.error('Failed to create business for OAuth user', { error: businessError, userId });
    throw new CustomError('Failed to create business', 500, 'BUSINESS_CREATE_ERROR');
  }
  ...
  logger.info('OAuth user registered successfully', { userId, email, businessId });
