...
import { CustomError, ValidationError, AuthenticationError, ConflictError, asyncHandler } from '../utils/errorHandler';
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Create a default agent for the business
    ...
    const { error: agentError } = await supabaseAdmin.from('agents').insert({
      ...
      name: 'Main Assistant',
      type: 'service',
      prompt: `You are a helpful AI assistant for ${businessName}. You help answer customer questions, take orders, and provide information about our business. Be friendly, professional, and concise.`,
    ...
    if (agentError) {
      logger.warn('Failed to create default agent', { error: agentError, businessId });
      // Don't fail signup if agent creation fails
    }
    ...
    if (!agentError) {
      logDatabase('INSERT', 'agents', { agentId, businessId });
    }
